...
/* allocate a char 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
	...
	if (!t) nrerror("allocation failure 1 in c3d()");
	t += NR_END;
	t -= nrl;

	/* allocate pointers to rows and set pointers to them */
	t[nrl]=(char **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(char*)));
	if (!t[nrl]) nrerror("allocation failure 2 in c3d()");
	t[nrl] += NR_END;
	t[nrl] -= ncl;

	/* allocate rows and set pointers to them */
	t[nrl][ncl]=(char *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(char)));
	if (!t[nrl][ncl]) nrerror("allocation failure 3 in c3d()");
	t[nrl][ncl] += NR_END;
	t[nrl][ncl] -= ndl;
...
void free_c3d(char ***t, long nrl, long nrh, long ncl, long nch,\
	long ndl, long ndh)
/* free a float f3tensor allocated by f3tensor() */
{
	free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));
